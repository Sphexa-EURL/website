        ...
        <div className="flex flex-col items-center justify-center space-y-4 text-center">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 mt-8 gap-8">
            {landingPageConfig.services.map((service) => (
              <Card
                className="transition-all hover:scale-105 duration-200 ease-in-out border rounded-md group shadow-none flex flex-col gap-2"
                key={service.title}
              >
                <CardHeader className="flex flex-col gap-2">
                  <service.icon className="group-hover:text-sphexa w-8 h-8 transition-all" />
                  <CardTitle className="text-left">{service.title}</CardTitle>
                </CardHeader>
                <CardContent>
